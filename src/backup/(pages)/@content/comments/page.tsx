  ...
  return (
    <section className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      {comments.map(comment => (
        <article key={comment.id} className="shadow  bg-transparent border-slate-600 border-2 p-2 rounded">
          <div className="flex items-center gap-2">
            <img src={comment.avatar || ""} alt="" className="h-20 w-20 rounded-full" />
            <div className="flex flex-col">
              <h2 className="font-semibold capitalize text-white">{comment.author_name}</h2>
